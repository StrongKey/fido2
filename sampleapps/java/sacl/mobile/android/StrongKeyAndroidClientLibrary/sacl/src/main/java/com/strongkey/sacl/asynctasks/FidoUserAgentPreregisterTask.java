 ...
 * **********************************************
 *
 * Asynchronous task to preregister with a FIDO server to get a challenge from
 * the FIDO server.
 */
...
public class FidoUserAgentPreregisterTask implements Callable {
     ...
     */
    public FidoUserAgentPreregisterTask(Context context, int did, Long uid) {
    ...
    /**
     * Thread that runs in the background. Generates parameters to call the preregister
     * webservice; verifies response and returns a PreregisterChallenge, or a JSONObject
    ...
    /**
     * Create preregister request with user/device credentials from the app; the SFA
     * application back-end is expected to retrieve them from the SFA's database and
     * relay the request to the FIDO server. Input here now looks like:
     *
     * {
     *  "saclFidoServiceInput": {
     *      "did": 1,
     *      "service": Constants.SACL_FIDO_SERVICES.SACL_FIDO_SERVICE_GET_FIDO_REGISTRATION_CHALLENGE,
     *      "saclCredentials": {
     *          "uid": 87
     *      }
     *   }
