...
3.  **Download** the binary distribution file [fido2server-v0.9.1-dist.tgz](https://github.com/StrongKey/fido2/raw/master/fido2server-v0.9.1-dist.tgz).

    ```
    shell> wget https://github.com/StrongKey/fido2/raw/master/fido2server-v0.9.1-dist.tgz
    ```

4.  **Extract the downloaded file to the current directory**:

    ```
    shell> tar xvzf fido2server-v0.9.1-dist.tgz
    ```
...
7. Using the following command, **confirm the FIDO2 Server is running**. The API _Web Application Definition Language (WADL)_ file comes back in response.
    ...
    ```
    shell> curl -k https://localhost:8181/api/application.wadl
    ```
