    ...
    public Response execute(Long did, Long sid, Long pid) {
        ...
        if (applianceCommon.replicate()) {
            if (!Boolean.valueOf(SKFSCommon.getConfigurationProperty("skfs.cfg.property.replicate.hashmapsonly"))) {
                ...
                if (response != null) {
                    return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(SKFSCommon.getMessageProperty("FIDOJPA-ERR-2006") + " deleteFidoPolicy replication error: " + response).build();
                }
            }
        }
