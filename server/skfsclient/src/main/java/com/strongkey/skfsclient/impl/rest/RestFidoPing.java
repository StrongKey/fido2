...
public class RestFidoPing {

    public static void ping(String REST_URI,
                                int did,
                                String authtype,
                                String credential1,
                                String credential2) throws Exception
    {
        ...
        */
        ...
        System.out.println("REST Ping test with " + authtype);
        System.out.println("******************************************");
        ...
        ping.getSVCInfo().setDid(did);
        ping.getSVCInfo().setProtocol(Constants.PROTOCOL_FIDO);
        if (authtype.equalsIgnoreCase(Constants.AUTHORIZATION_HMAC)) {
            ping.getSVCInfo().setAuthtype(Constants.AUTHORIZATION_HMAC);
        } else {
            ping.getSVCInfo().setAuthtype(Constants.AUTHORIZATION_PASSWORD);
            ping.getSVCInfo().setSVCUsername(credential1);
            ping.getSVCInfo().setSVCPassword(credential2);
        }
        ...
        String resourceLoc = REST_URI + Constants.REST_SUFFIX + Constants.REST_PING_ENDPOINT;
        ...
        try {
            ...
            switch (responseStatusLine.getStatusCode()) {
                case 200:
                    break;
                case 401:
                    System.out.println("Error during ping : 401 HMAC Authentication Failed");
                    return;
                case 404:
                    System.out.println("Error during ping : 404 Resource not found");
                    return;
                case 400:
                case 500:
                default:
                    System.out.println("Error during ping : " + responseStatusLine.getStatusCode() + " " + result);
                    return;
            }
